...
library(MASS)
# KoNLP 사용을 위한 
path <- file.path("C:/Users/안운빈/Desktop/학교/3-2/알고리즘/LDA/txt")  # 폴더 경로를 객체로 만든다
kor <- list.files(file.path(path, "2018"))  #폴더 경로 중 eng라는 폴더에 있는 파일들 이름을 list-up해서 객체로 만든다.
kor.files <- file.path(path, "2018", kor)   #아까 list-up한 파일의 이름들로 폴더의 경로를 다시 객체로 만든다.
